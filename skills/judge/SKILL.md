...
**2. The RUN.md** — the one file matching glob `*-workspace\RUN.md` under `~\.claude\runs\<session_id>\` (user-global default, OUT of any project tree; the session folder is injected by the UserPromptSubmit hook — one per session; Stop-gate v3.2 scopes enforcement to it). Override via env `AUTOWIN_RUN_ROOT`. Fallback: the legacy per-project `<cwd>\Audit\workspaces\<session_id>\` if present (transition). No machine-specific absolute path baked into the skill.
