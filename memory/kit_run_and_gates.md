...
**One work item = one `RUN.md`** (under `Audit\workspaces\<session_id>\<subject>-workspace\`). Machine-parseable header:
...
**The Stop-gate** (Stop hook) blocks end-of-turn unless every owned RUN is `green` (VERIFIED) or `degraded-closed` (USER-OK). A "green" is not believed — the gate **re-runs** the proof. It enforces only THIS session's runs (placed under `Audit\workspaces\<my id>\` or carrying `session:`).
